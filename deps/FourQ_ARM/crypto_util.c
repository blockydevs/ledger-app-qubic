...
bool is_zero_ct(digit_t* a, const unsigned int nwords) {  // Check if multiprecision element is zero
    digit_t x;
    unsigned int i;
...
void encode(
    point_t P,
    unsigned char* Pencoded) {  // Encode point P
...
ECCRYPTO_STATUS decode(const unsigned char* Pencoded,
                       point_t P) {  // Decode point P
                                     // SECURITY NOTE: this function does not run in constant time.
    ...
    if (sign != (unsigned int) sign_dec) {  // If sign of x-coordinate decoded != input sign bit,
                                            // then negate x-coordinate
        fp2neg1271(P->x);
    }
...
void to_Montgomery(const digit_t* ma, digit_t* c) {  // Converting to Montgomery representation

    Montgomery_multiply_mod_order(ma, (digit_t*) &Montgomery_Rprime, c);
}

void from_Montgomery(const digit_t* a,
                     digit_t* mc) {  // Converting from Montgomery to standard representation
...
const char* FourQ_get_error_message(
    const ECCRYPTO_STATUS Status) {  // Output error/success message for a given ECCRYPTO_STATUS
