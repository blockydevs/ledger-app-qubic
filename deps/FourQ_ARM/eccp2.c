...
void fp2copy1271(f2elm_t a, f2elm_t c) {  // Copy of a GF(p^2) element, c = a
    fpcopy1271(a[0], c[0]);
    fpcopy1271(a[1], c[1]);
}

void fp2zero1271(f2elm_t a) {  // Zeroing a GF(p^2) element, a = 0
    fpzero1271(a[0]);
    fpzero1271(a[1]);
}

void fp2neg1271(f2elm_t a) {  // GF(p^2) negation, a = -a in GF((2^127-1)^2)
    fpneg1271(a[0]);
    fpneg1271(a[1]);
}

void fp2sqr1271(f2elm_t a, f2elm_t c) {  // GF(p^2) squaring, c = a^2 in GF((2^127-1)^2)
    felm_t t1, t2, t3;
...
void fp2mul1271(f2elm_t a,
                f2elm_t b,
                f2elm_t c) {  // GF(p^2) multiplication, c = a*b in GF((2^127-1)^2)
    felm_t t1, t2, t3, t4;
...
__inline void fp2add1271(f2elm_t a,
                         f2elm_t b,
                         f2elm_t c) {  // GF(p^2) addition, c = a+b in GF((2^127-1)^2)
    fpadd1271(a[0], b[0], c[0]);
    fpadd1271(a[1], b[1], c[1]);
}

__inline void fp2sub1271(f2elm_t a,
                         f2elm_t b,
                         f2elm_t c) {  // GF(p^2) subtraction, c = a-b in GF((2^127-1)^2)
    fpsub1271(a[0], b[0], c[0]);
    fpsub1271(a[1], b[1], c[1]);
}

void fp2inv1271(f2elm_t a) {  // GF(p^2) inversion, a = (a0-i*a1)/(a0^2+a1^2)
    f2elm_t t1;
...
__inline void clear_words(
    void* mem,
    unsigned int
        nwords) {  // Clear integer-size digits from memory. "nwords" indicates the number of
                   // integer digits to be zeroed. This function uses the volatile type qualifier to
                   // inform the compiler not to optimize out the memory clearing. It has been
                   // tested with MSVS 2013 and GNU GCC 4.6.3, 4.7.3, 4.8.2 and 4.8.4. Users are
                   // responsible for verifying correctness with different compilers. See "Compliant
                   // Solution (C99)" at
                   // https://www.securecoding.cert.org/confluence/display/c/MSC06-C.+Beware+of+compiler+optimizations
    unsigned int i;
    volatile unsigned int* v = mem;

    for (i = 0; i < nwords; i++) v[i] = 0;
}
...
static uint64_t cpsi4[4] = {0xFFFFFFFFFFFFFFF6,
                            0x7FFFFFFFFFFFFFF9,
                            0xC59195418A18C59E,
                            0x334D90E9E28296F9};
...
static __inline void ecc_tau(point_extproj_t P) {  // Apply tau mapping to a point, P = tau(P)
                                                   // Input: P = (X1:Y1:Z1) on E in twisted Edwards
                                                   // coordinates Output: P = (Xfinal:Yfinal:Zfinal)
                                                   // on Ehat in twisted Edwards coordinates
    f2elm_t t0, t1;
...
static __inline void ecc_tau_dual(
    point_extproj_t P) {  // Apply tau_dual mapping to a point, P = tau_dual(P)
                          // Input: P = (X1:Y1:Z1) on Ehat in twisted Edwards coordinates
                          // Output: P = (Xfinal,Yfinal,Zfinal,Tafinal,Tbfinal) on E, where Tfinal =
                          // Tafinal*Tbfinal,
                          //         corresponding to (Xfinal:Yfinal:Zfinal:Tfinal) in extended
...
static __inline void ecc_delphidel(
    point_extproj_t
        P) {  // Apply delta_phi_delta mapping to a point, P = delta(phi_W(delta_inv(P))),
...
static __inline void ecc_delpsidel(
    point_extproj_t
        P) {  // Apply delta_psi_delta mapping to a point, P = delta(psi_W(delta_inv(P))),
...
void ecc_psi(point_extproj_t P) {  // Apply psi mapping to a point, P = psi(P)
                                   // Input: P = (X1:Y1:Z1) on E in twisted Edwards coordinates
                                   // Output: P = (Xfinal,Yfinal,Zfinal,Tafinal,Tbfinal) on E, where
                                   // Tfinal = Tafinal*Tbfinal,
                                   //         corresponding to (Xfinal:Yfinal:Zfinal:Tfinal) in
...
void ecc_phi(point_extproj_t P) {  // Apply phi mapping to a point, P = phi(P)
                                   // Input: P = (X1:Y1:Z1) on E in twisted Edwards coordinates
                                   // Output: P = (Xfinal,Yfinal,Zfinal,Tafinal,Tbfinal) on E, where
                                   // Tfinal = Tafinal*Tbfinal,
                                   //         corresponding to (Xfinal:Yfinal:Zfinal:Tfinal) in
...
static __inline void mul_truncate(
    uint64_t* s,
    uint64_t* C,
    uint64_t* out) {  // 256-bit multiplication with truncation for the scalar decomposition
...
void ecc_precomp(
    point_extproj_t P,
    point_extproj_precomp_t*
        T) {  // Generation of the precomputation table used by the variable-base scalar
              // multiplication ecc_mul(). Input: P = (X1,Y1,Z1,Ta,Tb), where T1 = Ta*Tb,
              // corresponding to (X1:Y1:Z1:T1) in extended twisted Edwards coordinates Output:
              // table T containing 8 points: P, P+phi(P), P+psi(P), P+phi(P)+psi(P), P+psi(phi(P)),
              // P+phi(P)+psi(phi(P)), P+psi(P)+psi(phi(P)), P+phi(P)+psi(P)+psi(phi(P)) Precomputed
              // points use the representation (X+Y,Y-X,2Z,2dT) corresponding to (X:Y:Z:T) in
              // extended twisted Edwards coordinates
...
void decompose(uint64_t* k,
               uint64_t* scalars) {  // Scalar decomposition for the variable-base scalar
                                     // multiplication Input: scalar in the range [0, 2^256-1].
    ...
    temp = (uint64_t) k[0] - (uint64_t) a1 * b11 - (uint64_t) a2 * b21 - (uint64_t) a3 * b31 -
           (uint64_t) a4 * b41 + c1;
    mask = ~(0 - (temp & 1));  // If temp is even then mask = 0xFF...FF, else mask = 0

    scalars[0] = temp + (mask & b41);
    scalars[1] = (uint64_t) a1 * b12 + (uint64_t) a2 - (uint64_t) a3 * b32 - (uint64_t) a4 * b42 +
                 c2 + (mask & b42);
    scalars[2] = (uint64_t) a3 * b33 - (uint64_t) a1 * b13 - (uint64_t) a2 + (uint64_t) a4 * b43 +
                 c3 - (mask & b43);
    scalars[3] = (uint64_t) a1 * b14 - (uint64_t) a2 * b24 - (uint64_t) a3 * b34 +
                 (uint64_t) a4 * b44 + c4 - (mask & b44);
}
...
void recode(
    ...
    unsigned int* digits,
    unsigned int*
        sign_masks) {  // Recoding sub-scalars for use in the variable-base scalar multiplication.
                       // See Algorithm 1 in "Efficient and Secure Methods for GLV-Based Scalar
                       // Multiplication and their Implementation on GLV-GLS Curves (Extended
                       // Version)", A. Faz-Hernandez, P. Longa, and A.H. Sanchez, in Journal of
                       // Cryptographic Engineering, Vol. 5(1), 2015. Input: 4 64-bit sub-scalars
                       // passed through "scalars", which are obtained after calling decompose().
                       // Outputs: "digits" array with 65 nonzero entries. Each entry is in the
                       // range [0, 7], corresponding to one entry in the precomputed table.
                       //          "sign_masks" array with 65 entries storing the signs for their
                       //          corresponding digits in "digits". Notation: if the corresponding
                       //          digit > 0 then sign_mask = 0xFF...FF, else if digit < 0 then
                       //          sign_mask = 0.
...
void cofactor_clearing(
    point_extproj_t
        P) {  // Co-factor clearing
              // Input: P = (X1,Y1,Z1,Ta,Tb), where T1 = Ta*Tb, corresponding to (X1:Y1:Z1:T1) in
              // extended twisted Edwards coordinates Output: P = 392*P =
              // (Xfinal,Yfinal,Zfinal,Tafinal,Tbfinal), where Tfinal = Tafinal*Tbfinal,
              //         corresponding to (Xfinal:Yfinal:Zfinal:Tfinal) in extended twisted Edwards
              //         coordinates
    ...
    eccadd(Q,
           P);  // P = P+Q using representations (X,Y,Z,Ta,Tb) <- (X,Y,Z,Ta,Tb) + (X+Y,Y-X,2Z,2dT)
    eccdouble(P);
    eccdouble(P);
    eccdouble(P);
    eccdouble(P);
...
bool ecc_mul(
    ...
    digit_t* k,
    point_t Q,
    bool clear_cofactor) {  // Variable-base scalar multiplication Q = k*P using a 4-dimensional
                            // decomposition Inputs: scalar "k" in [0, 2^256-1],
                            //         point P = (x,y) in affine coordinates,
                            //         clear_cofactor = 1 (TRUE) or 0 (FALSE) whether cofactor
                            //         clearing is required or not, respectively.
                            // Output: Q = k*P in affine coordinates (x,y).
                            // This function performs point validation and (if selected) cofactor
                            // clearing.
    point_extproj_t R;
...
void eccset(point_t P) {  // Set generator
                          // Output: P = (x,y)
...
void eccnorm(point_extproj_t P,
             point_t Q) {  // Normalize a projective point (X1:Y1:Z1), including full reduction
                           // Input: P = (X1:Y1:Z1) in twisted Edwards coordinates
                           // Output: Q = (X1/Z1,Y1/Z1), corresponding to (X1:Y1:Z1:T1) in extended
                           // twisted Edwards coordinates
...
void R1_to_R2(
    point_extproj_t P,
    point_extproj_precomp_t
        Q) {  // Conversion from representation (X,Y,Z,Ta,Tb) to (X+Y,Y-X,2Z,2dT), where T = Ta*Tb
              // Input:  P = (X1,Y1,Z1,Ta,Tb), where T1 = Ta*Tb, corresponding to (X1:Y1:Z1:T1) in
              // extended twisted Edwards coordinates Output: Q = (X1+Y1,Y1-X1,2Z1,2dT1)
              // corresponding to (X1:Y1:Z1:T1) in extended twisted Edwards coordinates
...
__inline void R1_to_R3(
    point_extproj_t P,
    point_extproj_precomp_t
        Q) {  // Conversion from representation (X,Y,Z,Ta,Tb) to (X+Y,Y-X,Z,T), where T = Ta*Tb
              // Input:  P = (X1,Y1,Z1,Ta,Tb), where T1 = Ta*Tb, corresponding to (X1:Y1:Z1:T1) in
              // extended twisted Edwards coordinates Output: Q = (X1+Y1,Y1-X1,Z1,T1) corresponding
              // to (X1:Y1:Z1:T1) in extended twisted Edwards coordinates
...
void R2_to_R4(
    ...
    point_extproj_t Q) {  // Conversion from representation (X+Y,Y-X,2Z,2dT) to (2X,2Y,2Z,2dT)
                          // Input:  P = (X1+Y1,Y1-X1,2Z1,2dT1) corresponding to (X1:Y1:Z1:T1) in
                          // extended twisted Edwards coordinates Output: Q = (2X1,2Y1,2Z1)
                          // corresponding to (X1:Y1:Z1) in twisted Edwards coordinates
...
void eccdouble(point_extproj_t P) {  // Point doubling 2P
                                     // Input: P = (X1:Y1:Z1) in twisted Edwards coordinates
                                     // Output: 2P = (Xfinal,Yfinal,Zfinal,Tafinal,Tbfinal), where
                                     // Tfinal = Tafinal*Tbfinal,
                                     //         corresponding to (Xfinal:Yfinal:Zfinal:Tfinal) in
                                     //         extended twisted Edwards coordinates
    f2elm_t t1, t2;
...
__inline void eccadd_core(
    point_extproj_precomp_t P,
    point_extproj_precomp_t Q,
    point_extproj_t
        R) {  // Basic point addition R = P+Q or R = P+P
              // Inputs: P = (X1+Y1,Y1-X1,2Z1,2dT1) corresponding to (X1:Y1:Z1:T1) in extended
              // twisted Edwards coordinates
              //         Q = (X2+Y2,Y2-X2,Z2,T2) corresponding to (X2:Y2:Z2:T2) in extended twisted
              //         Edwards coordinates
...
void eccadd(point_extproj_precomp_t Q,
            point_extproj_t P) {  // Complete point addition P = P+Q or P = P+P
                                  // Inputs: P = (X1,Y1,Z1,Ta,Tb), where T1 = Ta*Tb, corresponding
                                  // to (X1:Y1:Z1:T1) in extended twisted Edwards coordinates
                                  //         Q = (X2+Y2,Y2-X2,2Z2,2dT2) corresponding to
                                  //         (X2:Y2:Z2:T2) in extended twisted Edwards coordinates
                                  // Output: P = (Xfinal,Yfinal,Zfinal,Tafinal,Tbfinal), where
                                  // Tfinal = Tafinal*Tbfinal,
                                  //         corresponding to (Xfinal:Yfinal:Zfinal:Tfinal) in
                                  //         extended twisted Edwards coordinates
...
void point_setup(
    point_t P,
    point_extproj_t Q) {  // Point conversion to representation (X,Y,Z,Ta,Tb)
                          // Input: P = (x,y) in affine coordinates
                          // Output: P = (X,Y,1,Ta,Tb), where Ta=X, Tb=Y and T=Ta*Tb, corresponding
                          // to (X:Y:Z:T) in extended twisted Edwards coordinates
...
bool ecc_point_validate(
    point_extproj_t P) {  // Point validation: check if point lies on the curve
                          // Input: P = (x,y) in affine coordinates, where x, y in [0, 2^127-1].
                          // Output: TRUE (1) if point lies on the curve E: -x^2+y^2-1-dx^2*y^2 = 0,
                          // FALSE (0) otherwise. SECURITY NOTE: this function does not run in
                          // constant time (input point P is assumed to be public).
    f2elm_t t1, t2, t3;
    ...
    return ((is_digit_zero_ct(t1[0][0] | t1[0][1] | t1[0][2] | t1[0][3]) ||
             is_digit_zero_ct((t1[0][0] + 1) | (t1[0][1] + 1) | (t1[0][2] + 1) | (t1[0][3] + 1))) &&
            (is_digit_zero_ct(t1[1][0] | t1[1][1] | t1[1][2] | t1[1][3]) ||
             is_digit_zero_ct((t1[1][0] + 1) | (t1[1][1] + 1) | (t1[1][2] + 1) | (t1[1][3] + 1))));
}

static __inline void R5_to_R1(
    point_precomp_t P,
    point_extproj_t
        Q) {  // Conversion from representation (x+y,y-x,2dt) to (X,Y,Z,Ta,Tb)
              // Input:  P = (x1+y1,y1-x1,2dt1) corresponding to (X1:Y1:Z1:T1) in extended twisted
              // Edwards coordinates, where Z1=1 Output: Q = (x1,y1,z1,x1,y1), where z1=1,
              // corresponding to (X1:Y1:Z1:T1) in extended twisted Edwards coordinates
...
static __inline void eccmadd(
    point_precomp_t Q,
    point_extproj_t
        P) {  // Mixed point addition P = P+Q or P = P+P
              // Inputs: P = (X1,Y1,Z1,Ta,Tb), where T1 = Ta*Tb, corresponding to (X1:Y1:Z1:T1) in
              // extended twisted Edwards coordinates
              //         Q = (x2+y2,y2-x2,2dt2) corresponding to (X2:Y2:Z2:T2) in extended twisted
              //         Edwards coordinates, where Z2=1
...
bool ecc_mul_fixed(
    digit_t* k,
    point_t Q) {  // Fixed-base scalar multiplication Q = k*G, where G is the generator.
                  // FIXED_BASE_TABLE stores v*2^(w-1) = 80 multiples of G. Inputs: scalar "k" in
                  // [0, 2^256-1]. Output: Q = k*G in affine coordinates (x,y). The function is
                  // based on the modified LSB-set comb method, which converts the scalar to an odd
                  // signed representation with (bitlength(order)+w*v) digits.
    ...
    table_lookup_fixed_base(((point_precomp_t*) &FIXED_BASE_TABLE) + (v - 1) * (1 << (w - 1)),
                            S,
                            digit,
                            digits[d - 1]);
...
void mLSB_set_recode(
    uint64_t* scalar,
    unsigned int*
        digits) {  // Computes the modified LSB-set representation of a scalar
                   // Inputs: scalar in [0, order-1], where the order of FourQ's subgroup is 246
                   // bits. Output: digits, where the first "d" values (from index 0 to (d-1)) store
                   // the signs for the recoded values using the convention: -1 (negative), 0
                   // (positive), and
                   //         the remaining values (from index d to (l-1)) store the recoded values
                   //         in mLSB-set representation, excluding their sign, where l = d*w and d
                   //         = ceil(bitlength(order)/(w*v))*v. The values v and w are fixed and
                   //         must be in the range [1, 10] (see FourQ.h); they determine the size of
                   //         the precomputed table "FIXED_BASE_TABLE" used by ecc_mul_fixed().
    ...
    for (i = 0; i < (d - 1); i++) {
        digits[i] = (unsigned int) ((scalar[0] & 1) -
                                    1);  // Convention for the "sign" row:
                                         // if scalar_(i+1) = 0 then digit_i = -1 (negative), else
                                         // if scalar_(i+1) = 1 then digit_i = 0 (positive)
        // Shift scalar to the right by 1
        for (j = 0; j < (NWORDS64_ORDER - 1); j++) {
            SHIFTR(scalar[j + 1], scalar[j], 1, scalar[j], RADIX64);
        }
    ...
    for (i = d; i < l; i++) {
        digits[i] = (unsigned int) (scalar[0] &
                                    1);  // digits_i = k mod 2. Sign is determined by the "sign" row
        ...
        carry = (temp &
                 (uint64_t) is_digit_zero_ct((digit_t) scalar[0]));  // carry = (scalar[0] < temp);
...
#if (USE_ENDO == true)
static __inline void eccneg_extproj_precomp(
    point_extproj_precomp_t P,
    point_extproj_precomp_t Q) {  // Point negation
...
static __inline void eccneg_precomp(
    point_precomp_t P,
    point_precomp_t Q) {  // Point negation
...
bool ecc_mul_double(
    ...
    point_t Q,
    digit_t* l,
    point_t R) {  // Double scalar multiplication R = k*G + l*Q, where the G is the generator. Uses
                  // DOUBLE_SCALAR_TABLE, which contains multiples of G, Phi(G), Psi(G) and
                  // Phi(Psi(G)). Inputs: point Q in affine coordinates,
...
#define USE_ENDO false
    ...
    for (i = 64; i >= 0; i--) {
        ...
        if (digits_l1[i] < 0) {
            ...
            eccneg_extproj_precomp(Q_table1[position],
                                   U);  // Load and negate U = (X_U,Y_U,Z_U,Td_U) <-
            ...
            position = (digits_l1[i]) / 2;  // Take U = (X_U,Y_U,Z_U,Td_U) <- (X+Y,Y-X,2Z,2dT) from
                                            // a point in the precomputed table
        ...
        if (digits_k1[i] < 0) {
            ...
            eccneg_precomp(((point_precomp_t*) &DOUBLE_SCALAR_TABLE)[position],
                           V);  // Load and negate V = (X_V,Y_V,Z_V,Td_V) <- -(x+y,y-x,2dt) from a
            ...
            position = (digits_k1[i]) / 2;  // Take V = (X_V,Y_V,Z_V,Td_V) <- (x+y,y-x,2dt) from a
                                            // point in the precomputed table
            eccmadd(((point_precomp_t*) &DOUBLE_SCALAR_TABLE)[position],
        ...
        if (digits_k2[i] < 0) {
            ...
            eccneg_precomp(
                ((point_precomp_t*) &DOUBLE_SCALAR_TABLE)[NPOINTS_DOUBLEMUL_WP + position],
                V);
            eccmadd(V, T);
        ...
        if (digits_k3[i] < 0) {
            ...
            eccneg_precomp(
                ((point_precomp_t*) &DOUBLE_SCALAR_TABLE)[2 * NPOINTS_DOUBLEMUL_WP + position],
                V);
            eccmadd(V, T);
        } else if (digits_k3[i] > 0) {
            position = (digits_k3[i]) / 2;
            eccmadd(((point_precomp_t*) &DOUBLE_SCALAR_TABLE)[2 * NPOINTS_DOUBLEMUL_WP + position],
                    T);
        }
        if (digits_k4[i] < 0) {
            position = (-digits_k4[i]) / 2;
            eccneg_precomp(
                ((point_precomp_t*) &DOUBLE_SCALAR_TABLE)[3 * NPOINTS_DOUBLEMUL_WP + position],
                V);
            eccmadd(V, T);
        } else if (digits_k4[i] > 0) {
            position = (digits_k4[i]) / 2;
            eccmadd(((point_precomp_t*) &DOUBLE_SCALAR_TABLE)[3 * NPOINTS_DOUBLEMUL_WP + position],
                    T);
        }
    }
...
void ecc_precomp_double(
    point_extproj_t P,
    point_extproj_precomp_t* Table,
    unsigned int npoints) {  // Generation of the precomputation table used internally by the double
                             // scalar multiplication function ecc_mul_double(). Inputs: point P in
                             // representation (X,Y,Z,Ta,Tb),
                             //         Table with storage for npoints,
                             //         number of points "npoints".
                             // Output: Table containing multiples of the base point P using
                             // representation (X+Y,Y-X,2Z,2dT).
    point_extproj_t Q;
...
void wNAF_recode(uint64_t scalar,
                 unsigned int w,
                 int* digits) {  // Computes wNAF recoding of a scalar, where digits are in set
                                 // {0,+-1,+-3,...,+-(2^(w-1)-1)}
    unsigned int i;
    int digit, index = 0;
